...
export default defineEventHandler(async (event) => {
	...
	try {
		...
		const mediumPathData = interMedium.getPath(lineOne.toString(), 42, 26.5, 17);
		...
		const extraBoldPathData = interExtraBold.getPath(lineTwo.toString(), mediumWidth + 42 + 3.6, 26.5, 17);
